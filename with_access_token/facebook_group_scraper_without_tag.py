...
def getFacebookPageFeedData(page_id, access_token, since, unitl):
    ...
    ###### message_tag 추가
    parameters2 = "comments.summary(true).filter(stream){message,message_tags}"
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # tag가 여러개 일수도 있다(dict가 있는 list로 보내짐) 없으면 빈 string으로

                    #### tag 삭제하는 부분 추가
                    tag_infos = comments[i]['message_tags'] if 'message_tags' in comments[i] else []
                    # print(tag_infos)
                    # reduce 사용하고 dict 에서 name 만 뽑아서 저장하기
                    tag_names = reduce(lambda x, y: x+[y['name']], tag_infos, [])
                    message = comments[i]['message']
                    for tag_name in tag_names:
                        message = message.replace(tag_name, '')

                    com = com + ' :] ' + message.encode('cp949', errors= 'replace').decode('cp949')
                    i = i + 1
                except:
                    break
                    ############
